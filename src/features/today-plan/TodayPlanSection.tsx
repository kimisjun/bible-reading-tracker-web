...
export type TodayPlanSectionProps = Readonly<{
  ...
  onRead(planId: string, bookId: string, chapter: number): void
  onCompleteAll(planId: string, incomplete: ChapterRef[]): void
  onUndoBatch(planId: string, batchId: string): void
}>
      ...
      <div className="today-plan__cards">
        {views.map((view) => (
          ...
          >
                  ...
                  {chapter.completed ? (
                    ...
                    <>
                      ...
                      <button
                        ...
                        onClick={() => onRead(view.planId, chapter.bookId, chapter.chapter)}
                      >
                        읽었어요
                      </button>
                    </>
            ...
            {(view.statusMessage || view.lastScheduledDate || (view.chapters.length > 0 && view.chapters.every((chapter) => chapter.completed))) && (
              <div className="today-plan__status" role="status" aria-live="polite">
                ...
                {view.chapters.length > 0 && view.chapters.every((chapter) => chapter.completed) && (
            ...
            {(() => {
              ...
              return (
                <button
                  ...
                  onClick={() => onCompleteAll(view.planId, incomplete)}
                >
                  전체 완료
                </button>
              )
            ...
            {view.recentBatchId && (
              <button
                ...
                onClick={() => onUndoBatch(view.planId, view.recentBatchId!)}
              >
                전체 완료 취소
              </button>
            )}
