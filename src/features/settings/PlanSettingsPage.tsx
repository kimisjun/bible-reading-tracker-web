...
type PreviewResult = Readonly<{ plan: ReadingPlan; preview: PlanPreview } | { error: string }>
...
function WeekdayPicker({ id, value, onChange }: Readonly<{
  id: string
  value: readonly Weekday[]
  onChange: (weekdays: Weekday[]) => void
}>) {
  return (
    <fieldset className="plan-settings__fieldset">
      <legend>읽는 요일</legend>
...
function Preview({ result }: Readonly<{ result: PreviewResult }>) {
  if ('error' in result) return <p className="plan-settings__alert" role="alert">{result.error}</p>
...
export function PlanSettingsPage({
  ...
  const [commonPreset, setCommonPreset] = useState<PlanPreset>('one-year')
  const [commonStart, setCommonStart] = useState(today)
  const [commonWeekdays, setCommonWeekdays] = useState<Weekday[]>([...ALL_WEEKDAYS])
  const [commonOrder, setCommonOrder] = useState<ReadingOrder>('canonical')
  const [commonPolicy, setCommonPolicy] = useState<MissedDayPolicy>('carry')
    ...
    <div className="plan-settings">
      <section aria-labelledby="common-plan-title" className="plan-settings__card" role="region">
        ...
        <form onSubmit={saveCommon}>
          ...
          <input id="common-start" type="date" value={commonStart} onChange={(event) => setCommonStart(event.target.value)} />
          <WeekdayPicker id="common-weekdays" value={commonWeekdays} onChange={setCommonWeekdays} />
      ...
      <section aria-labelledby="personal-plan-title" className="plan-settings__card" role="region">
        ...
        <form onSubmit={savePersonal}>
          ...
          <input id="personal-name" value={personalName} onChange={(event) => setPersonalName(event.target.value)} />
          ...
          {personalRangeType === 'books' && (
            <fieldset className="plan-settings__fieldset plan-settings__books">
              <legend>읽을 책</legend>
          ...
          <div className="plan-settings__date-grid">
            <label htmlFor="personal-start">시작일<input id="personal-start" type="date" value={personalStart} onChange={(event) => setPersonalStart(event.target.value)} /></label>
            <label htmlFor="personal-end">종료일<input id="personal-end" type="date" value={personalEnd} onChange={(event) => setPersonalEnd(event.target.value)} /></label>
          </div>
          <WeekdayPicker id="personal-weekdays" value={personalWeekdays} onChange={setPersonalWeekdays} />
